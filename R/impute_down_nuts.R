...
impute_down_nuts <- function(
  ...
  get_valid_nuts_codes <- function(this_env) {
    data("all_valid_nuts_codes", package = "regions", envir = this_env)
    all_valid_nuts_codes
  }

  validated <- dat %>%
    rename(
      ## will be turned back on return, easier to handle
  ...
  validated_nuts_1 <- validated %>%
    filter(.data$typology == "nuts_level_1", .data$valid == TRUE)

  ## subset for selecting only valid NUTS2 geo codes for imputing down
  validated_nuts_2 <- validated %>%
    filter(.data$typology == "nuts_level_2", .data$valid == TRUE)
  ...
  all_valid_nuts_codes_year <- all_valid_nuts_codes_year %>%
    filter(.data$nuts == nuts_filter, .data$typology == "nuts_level_3") %>%
    mutate(
      ## start with smallest hierarchical unit, which is NUTS3
      country_code = get_country_code(.data$geo),
      row_id = seq_len(nrow(.))
    ) %>%
    pivot_wider(names_from = "typology", values_from = 'geo') %>%
    mutate(
  ...
  potentially_imputed_from_nuts_1 <- full_code_table %>%
    ...
    left_join(dat, by = 'geo') %>%
    mutate(
      method = paste0("imputed from NUTS1 ", .data$geo, " [", .data$method, "]")
    ) %>%
  ...
  potentially_imputed_from_nuts_2 <- full_code_table %>%
    ...
    left_join(dat, by = 'geo') %>%
    mutate(
      method = paste0("imputed from NUTS2 ", .data$geo, " [", .data$method, "]")
    ) %>%
  ...
  actual_to_imputed <- validated %>%
    full_join(imputed_from_nuts_2, by = c("geo", "values", "method"))
  ...
  imputed_dfv <- validate_nuts_regions(imputed_df_2, nuts_year = nuts_year) %>%
    dplyr::arrange(.data$geo) %>%
    mutate(
      ## remove case when method was originally "" and we have [] in the end
